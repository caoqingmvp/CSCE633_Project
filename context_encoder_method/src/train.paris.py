...
if not os.path.exists( trainset_path ) or not os.path.exists( testset_path ):
    ...
    testset_dir = os.path.join( dataset_path, 'paris_eval/paris_eval_gt' )
...
labels_D = tf.concat(axis=0, values=[tf.ones([batch_size]), tf.zeros([batch_size])] )
...
adversarial_all = tf.concat(axis=0, values=[adversarial_pos, adversarial_neg])
...
mask_recon = tf.concat(axis=2, values=[mask_recon]*3)
...
for epoch in range(n_epochs):
    ...
    for start,end in zip(
        ...
        if iters % 100 == 0:
            ...
            # Generate result every 100 iterations
            if iters % 100 == 0:
                ii = 0
